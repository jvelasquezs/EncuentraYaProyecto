...
const protect = async (req, res, next) => {
  ...
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
    try {
      ...
      const db = getDb();
      const store = db.prepare('SELECT id, nombreTienda, responsable, rif, telefono, correo, logo, contacto_whatsapp, contacto_instagram, descripcion, plataformas, monedas, latitud, longitud FROM stores WHERE id = ?').get(decoded.id);

      if (!store) {
        return res.status(401).json({ error: 'No autorizado, comercio no encontrado' });
      }

      req.user = { ...store, _id: store.id, rol: 'Vendedor' };
      next();
    } catch (error) {
